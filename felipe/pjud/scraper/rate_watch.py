...
def slots():
    return sorted(d for d in DATA.glob("worker_a*") if (d / "sweep.log").exists())
...
def scan(path, since):
    ...
    """
    counts = {"result": 0, "open": 0, "ebook": 0, "bad": 0}
    last = None
    try:
    ...
    for line in text.splitlines():
        for key, rx in (("result", RESULT_RE), ("open", OPEN_RE),
                        ("ebook", EBOOK_RE), ("bad", BAD_RE)):
            m = rx.match(line)
            if not m:
                continue
...
def report(mins):
    ...
    print(f"{'slot':<6}{'results':>9}{'opens':>7}{'ebooks':>8}{'req-equiv':>11}"
          f"{'req/min':>9}{'trouble':>9}  last")
    tot = {"result": 0, "open": 0, "ebook": 0, "bad": 0}
    for d in slots():
        c, last = scan(d / "sweep.log", since)
        if not any(c.values()):
            continue
        for k in tot:
            tot[k] += c[k]
        req = c["result"] + c["open"] + c["ebook"]
        print(f"{d.name[-2:]:<6}{c['result']:>9}{c['open']:>7}{c['ebook']:>8}{req:>11}"
              f"{req / mins:>9.2f}{c['bad']:>9}  {last:%H:%M:%S}" if last else "")
    req = tot["result"] + tot["open"] + tot["ebook"]
    print("-" * 68)
    print(f"{'ALL':<6}{tot['result']:>9}{tot['open']:>7}{tot['ebook']:>8}{req:>11}"
