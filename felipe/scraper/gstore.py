"""Google Sheets + Drive data layer for the JPL scraper.

Mirrors felipe/pjud/scraper/gstore.py (same engine; different tab layout). Replaces
the old Supabase checkpoints/relational tables + Apps Script export + Storage bucket:
  - The "database" is a Google Sheet with 8 tabs (one per entity / junction).
  - PDFs (Sección C trámites + Sección D documentos) go to a "Documentos" Drive
    subfolder; the Sheet stores the Drive link.
  - Writes are an incremental UPSERT keyed on column A (the entity ID).

`run.py --setup` calls provision() once and saves the IDs to jpl_config.json
(gitignored). Every later run loads that config; Store reads each tab's column A
to know which IDs already exist (this is also how resume works — a caso_id already
in the Causas tab is skipped).
"""
...
# ── Tab layout — column A is the entity key; matches the old export_sheets layout ──
TABS = {
    "Juzgados": ["juzgado_id", "nombre", "url"],
...
CONFIG_PATH = Path(__file__).resolve().parent / "jpl_config.json"
...
def _make_public_reader(drive, file_id):
    """Anyone-with-link can read — so the web page (public) can fetch the CSV / PDFs."""
    try:
...
def provision(creds=None):
    ...
    subfolder. Idempotent. Returns + saves the config."""
...
class Store:
    ...
    def upsert(self, table, rows):
        ...
        # Re-write only =formula cells USER_ENTERED (rest stays RAW so dates aren't coerced)
    ...
    def upload_pdf(self, object_path, data):
        """Upload PDF bytes to Documentos; return its Drive link. Skips re-upload
        if a file with the same flattened name already exists."""
        if len(data) < 1024:
